...
const App: React.FC = () => {
  ...
  const updateTasks = (newTasks: TaskItem[]) => {
    ...
    newTasks.forEach(task => {
      if (!task.isCompleted && task.dueDate) {
        NotificationService.scheduleTaskReminder(task.title, task.dueDate);
      }
    });
  };
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case 'ai-coach': return <AICoach tasks={tasks} onAIConsult={() => checkAchievements('ai', 1)} />;
  ...
  return (
    <div className="flex h-screen overflow-hidden transition-colors duration-300">
      ...
      {/* REWARD AD MODAL (Previously existing, kept intact) */}
      {isAdPlaying && (
